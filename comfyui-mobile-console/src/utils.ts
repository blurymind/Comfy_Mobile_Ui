...
export const load_api_workflows = (onDone: any) => {
	// fetch("http://localhost:8188/fireplace/workflows.json").then(res=>res.json()).then((data => {
	// 	console.log("----- FETCHING outputs", {data})
	// 	data
	// }))
	if (fetched) return;
	...
	fetch(`${COMFY_UI_URL}/fireplace/workflows.json`)
		...
		.then((data) => {
			...
			fetch("http://localhost:8188/fireplace/fs", {
				method: 'POST',
				headers: {
